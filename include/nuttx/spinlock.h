...
#define spin_is_locked(l) (*(l) == SP_LOCKED)
  ...
  static inline_function
      irqstate_t
      raw_spin_lock_irqsave(FAR volatile spinlock_t *lock)
  {
    irqstate_t flags;
    flags = up_irq_save();
...
#define raw_spin_lock_irqsave(l) ((void)(l), up_irq_save())
  ...
  static inline_function
      irqstate_t
      spin_lock_irqsave(FAR volatile spinlock_t *lock)
  {
    irqstate_t flags;
...
#define raw_spin_trylock_irqsave(l, f)  \
  ({                                    \
    f = up_irq_save();                  \
    raw_spin_trylock(l) ? true : ({ up_irq_restore(f); false; }); \
  })
#else
#define raw_spin_trylock_irqsave(l, f) \
  ({                                   \
...
#define spin_trylock_irqsave(l, f)  \
  ({                                \
    f = up_irq_save();              \
    spin_trylock(l) ? true : ({ up_irq_restore(f); false; }); \
  })
#else
#define spin_trylock_irqsave(l, f) \
  ({                               \
...
#ifdef CONFIG_SPINLOCK
  static inline_function void raw_spin_unlock_irqrestore(FAR volatile spinlock_t *lock,
                                                         irqstate_t flags)
  {
    raw_spin_unlock(lock);
...
#ifdef CONFIG_SPINLOCK
  static inline_function void spin_unlock_irqrestore(FAR volatile spinlock_t *lock, irqstate_t flags)
  {
    /* Unlock without trace note */
